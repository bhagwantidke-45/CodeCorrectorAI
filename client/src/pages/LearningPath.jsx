      ...
      <div style={{ minHeight: '100vh', background: '#0f0f1a', color: '#e2e8f0', fontFamily: 'Inter, sans-serif' }}>
        ...
        <div style={{ background: rm.gradient, borderBottom: '1px solid rgba(255,255,255,0.1)', padding: '28px 40px' }}>
          <button onClick={() => setSelectedRoadmap(null)}
            style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: '#fff', padding: '6px 14px', borderRadius: 8, cursor: 'pointer', fontSize: 13, marginBottom: 16 }}>
            ← Back to Roadmaps
          </button>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '28px 40px' }}>
          {rm.topics.map((topic, idx) => {
            ...
            const isLocked = idx > 0 && !completedTopics.has(rm.topics[idx - 1].id) && idx > done;
            return (
              <div key={topic.id} style={{ display: 'flex', gap: 16, marginBottom: 16, alignItems: 'flex-start' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 4 }}>
                  ...
                  {idx < rm.topics.length - 1 && (
                    <div style={{ width: 2, height: 32, background: isDone ? 'rgba(34,197,94,0.4)' : 'rgba(255,255,255,0.07)', margin: '4px 0' }} />
                  )}
                </div>

                {/* Card */}
                <div style={{ flex: 1, background: isDone ? 'rgba(34,197,94,0.06)' : 'rgba(255,255,255,0.03)', border: `1px solid ${isDone ? 'rgba(34,197,94,0.3)' : 'rgba(255,255,255,0.07)'}`, borderRadius: 14, padding: '16px 20px', transition: 'all 0.2s' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontSize: 15, fontWeight: 700, color: isDone ? '#22c55e' : '#e2e8f0' }}>{topic.label}</span>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      ...
                      <button onClick={() => startTopic(topic)}
                        style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 16px', borderRadius: 8, border: 'none', background: isDone ? 'rgba(34,197,94,0.2)' : `rgba(${rm.color.includes('22c') ? '34,197,94' : rm.color.includes('f59') ? '245,158,11' : rm.color.includes('818') ? '129,140,248' : '236,72,153'},0.2)`, color: isDone ? '#22c55e' : rm.color, fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
                        <PlayCircle size={13} /> {isDone ? 'Review' : 'Practice'}
                      </button>
                    </div>
                  </div>
                </div>
