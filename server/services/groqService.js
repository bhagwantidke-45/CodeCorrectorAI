...
export const analyzeCodeWithGroq = async (code, language) => {
  ...
  const completion = await groq.chat.completions.create({
    ...
    model: "llama-3.3-70b-versatile", // Using the newer Llama 3.3 model
    temperature: 0.2,
  ...
  // Strip markdown code blocks if the model ignores the prompt instruction
  const cleaned = text
    .replace(/^```json\s*/i, '')
    .replace(/^```\s*/i, '')
    .replace(/\s*```$/i, '')
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      throw new Error('Groq returned non-JSON response. Please try again.');
    }
  }
